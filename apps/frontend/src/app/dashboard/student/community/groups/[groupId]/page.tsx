...
export default function GroupChatPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on('presence:sync', ({ onlineUserIds: ids }: { onlineUserIds: string[] }) => {
        setOnlineUserIds(new Set([...ids, user.id])); // Always include self
      });
      ...
      socket.on('presence:offline', ({ userId }: { userId: string }) => {
        setOnlineUserIds(prev => {
          if (userId === user.id) return prev; // don't remove self
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      const res = await api.post(`/community/groups/${groupId}/messages`, {
        content: newMessage.trim(),
      });
      // Append optimistically just in case socket is slow
      setMessages(prev => {
  ...
  const onlineMembers = members.filter(m => m.user.id === user?.id || onlineUserIds.has(m.user.id));
  const offlineMembers = members.filter(m => m.user.id !== user?.id && !onlineUserIds.has(m.user.id));
  ...
  return (
    <div className="flex w-full h-[calc(100vh-64px)] bg-slate-50 overflow-hidden font-sans">
      
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col bg-white border-r border-slate-200/60 shadow-sm z-10 relative">
        
        {/* Header */}
        <div className="h-16 border-b border-slate-100 flex items-center px-6 gap-4 bg-white/80 backdrop-blur-md sticky top-0 z-20">
          <button 
            onClick={() => router.push('/dashboard/student/community/groups')} 
              ...
              return (
                <div key={msg.id} className={`flex gap-3 items-end ${isMe ? 'flex-row-reverse' : 'flex-row'} group`}>
                  ...
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 border border-slate-200 flex items-center justify-center shrink-0 overflow-hidden shadow-sm">
                    <img 
                      src={msg.sender.avatarUrl || `https://api.dicebear.com/7.x/notionists/svg?seed=${msg.sender.id}&backgroundColor=e2e8f0`} 
                      alt="" 
                  ...
                  <div className={`flex flex-col max-w-[75%] ${isMe ? 'items-end' : 'items-start'}`}>
                    <div className={`flex items-baseline gap-2 mb-1.5 px-1 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                      <span className="text-[12px] font-bold text-slate-700">{isMe ? 'You' : `${msg.sender.firstName} ${msg.sender.lastName}`}</span>
      ...
      <div className="w-[300px] shrink-0 flex flex-col bg-slate-50/50">
        <div className="p-6 border-b border-slate-200/60 flex items-center justify-between">
          <h3 className="m-0 text-sm font-extrabold text-slate-800 flex items-center gap-2 tracking-tight">
            <Users size={16} className="text-slate-400" /> Member Directory
          </h3>
          {isOwner && (
            <button 
