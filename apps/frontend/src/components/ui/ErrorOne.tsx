...
function PillButton({ label, icon, href, onClick }: ErrorOneAction) {
  const cls =
    'h-10 rounded-full px-6 text-sm font-semibold transition-opacity hover:opacity-90 mt-4 shadow-[inset_0_2px_4px_0_rgba(255,255,255,0.7),inset_0_-2px_4px_0_rgba(0,0,0,0.2)] sm:w-auto dark:shadow-[inset_0_-2px_4px_0_rgba(0,0,0,0.2),inset_0_2px_4px_0_rgba(255,255,255,0.5)] inline-flex items-center justify-center gap-1.5';
...
export function ErrorOne({
  ...
  return (
    <main className="theme-injected bg-background text-foreground relative flex min-h-screen items-center justify-center px-6 py-16">
      <div
        className="dark:opacity-20 mask-radial-at-center mask-radial-from-muted pointer-events-none absolute inset-0 z-0 mask-radial-to-transparent opacity-10"
        style={{
          backgroundImage: `
